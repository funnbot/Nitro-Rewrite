...
let guildMember = (member, type) => {
    ...
    const memberlog = bot.table("MemberLog").get(member.guild);
    let msg = memberlog[type ? "join" : "leave"];
    if (!msg) return;
    const channel = guild.channels.get(memberlog.channel);
    if (!channel ||
        channel.type !== "text" ||
        !channel.permissionsFor(bot.user).has("SEND_MESSAGES")) return;
    if (msg === "image") return handleImage(user, channel, msg, type);
...
function replace(str, user, channel) {
    let rep = {
        ...
        "{name}": user.username,
        "{tag}": user.tag,
        "{total}": channel.guild.members.size
    };
